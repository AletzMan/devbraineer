import { SquareTerminalIcon, FileJsonIcon, RegexIcon, AlertCircle } from 'lucide-react';
import { SiteFooter } from '../componentes/SiteFooter';

const menuData = [
    {
        label: 'Convertidor de Datos',
        description: 'Convierte formatos JSON, CSV y más de forma sencilla.',
    ...
    },
    {
        ...
        description: 'Crea y prueba expresiones regulares personalizadas.',
...
export default function ToolsDevHome() {
    return (
        <div className="max-w-(--max-width) mx-auto px-6 py-10 flex flex-col h-[calc(100svh-4rem)] flex-grow overflow-y-auto scrollbar-thin">
            <h1 className={`text-3xl font-semibold mb-10 flex items-center gap-3 ${menuData[0].color}`}>
                <SquareTerminalIcon className={`w-7 h-7 ${menuData[0].color}`} />
            ...
            <div className="grid gap-8 grid-cols-[repeat(auto-fill,minmax(340px,1fr))] flex-grow">
                {menuData.map(({ label, description, href, icon: Icon, color }) => (
                    <div
                        ...
                        <Icon
                            className="absolute bottom-4 right-4 size-18 opacity-10 pointer-events-none select-none text-gray-300"
                            aria-hidden="true"
                        />
                        <a
                            href={href}
