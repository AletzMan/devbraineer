...
export default function LayoutSection({ title }: LayoutSubSectionProps) {
    ...
    return (
        <div className="max-(--max-width) mx-auto p-4 flex flex-col h-[calc(100vh-5rem)] overflow-y-auto scrollbar-thin">
            <header className="bg-base-200 p-6 rounded-md mb-10">
                <h1 className={`text-3xl font-semibold mb-5 flex items-center gap-3 ${infoSection?.color}`}>
                    {Icon && <Icon className="w-7 h-7" />}
                    {title}
                </h1>
                <p className="text-sm text-gray-500">{description}</p>
            </header>
            <div className="relative grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 flex-grow auto-rows-fr">
                {infoSection?.children?.map(({ label, description, href, icon: Icon, color, children }) => (
                    <div
                        key={label}
