...
import { CircleOff, EllipsisIcon, SaveIcon, TrashIcon } from 'lucide-react';
        ...
        <aside className="bg-base-100 shadow-md rounded-sm p-2 h-[calc(100svh-9.5rem)] ">
            <header className="flex justify-between items-center mb-2 border-b border-gray-700 pb-2">
                ...
                <div className="tooltip" data-tip="Eliminar todo el historial">
                    <button className="btn btn-xs btn-ghost btn-circle btn-error" onClick={() => deleteHistory()}>
            ...
            <div className="overflow-y-auto scrollbar-thin h-[calc(100svh-13rem)] ">
                {history.length === 0 ? (
                    <p className="flex flex-col items-center text-gray-500/80 justify-center gap-2 text-md w-full text-center pt-9">
                    ...
                    <motion.div layoutRoot layout className="space-y-2 w-full">
                        {groupedHistory.map((dayGroup, index) => (
                            <div key={dayGroup.date} className="collapse collapse-arrow bg-base-200 rounded-sm w-full">
                                <input
                                    type="checkbox"
                                        ...
                                        <motion.ul layout className="flex flex-col gap-1">
                                            {dayGroup.entries.map((entry) => (
                                                <motion.li
                                                    ...
                                                    transition={{ duration: 0.3 }}
                                                    title={entry.url}
                                                    className="relative p-1 cursor-pointer bg-base-300 hover:bg-gray-700/20 px-2 rounded transition-colors duration-200"
                                                    onClick={() =>
                                                        setFormState({
                                                            method: entry.method,
                                                            url: entry.url,
