...
<script>
  ...
  `
    ...
    `
        ...
        // Crear un sistema de módulos simulado para manejar importaciones
        const moduleSystem = `
        <script>
          // Sistema de módulos simulado
          window.__modules = {};
          window.__exports = {};
          ...
          // Función para importar un módulo
          function requireModule(name) {
            if (!window.__exports[name]) {
              const exports = {};
              window.__modules[name](exports);
              window.__exports[name] = exports;
            }
            return window.__exports[name];
          }
        </script>
        `

        // Procesar los archivos para manejar importaciones
        const processedFiles = reactFiles.map((file) => {
          ...
          content = content.replace(/export\s+default\s+(\w+)/g, "exports.default = $1")
          content = content.replace(/export\s+const\s+(\w+)/g, "exports.$1 = $1")
        ...
        <body>
          ...
          ${processedFiles
            .filter((file) => file.id !== mainFile.id)
            .map(
              (file) => `
...
.feature h2 {
  ...
  // Actualizar archivos React para soportar importaciones
  useEffect(() => {
    // Actualizar el archivo Header.jsx para ser importable
    const reactJsHeaderFile = files.find((file) => file.id === "react-js-2")
    if (reactJsHeaderFile && !reactJsHeaderFile.content.includes("export default Header")) {
      setFiles(
        files.map((file) => {
          if (file.id === "react-js-2") {
            return {
              ...file,
              content: `// Componente Header para React
function Header({ title }) {
...
export default Header;`,
    ...
    // Actualizar el archivo Header.tsx para ser importable
    const reactTsHeaderFile = files.find((file) => file.id === "react-ts-2")
    if (reactTsHeaderFile && !reactTsHeaderFile.content.includes("export default Header")) {
      setFiles(
        files.map((file) => {
          if (file.id === "react-ts-2") {
            return {
              ...file,
              content: `// Componente Header para React con TypeScript
interface HeaderProps {
...
export default Header;`,
    ...
    // Actualizar el archivo App.jsx para incluir importaciones
    const reactJsAppFile = files.find((file) => file.id === "react-js-1")
    if (reactJsAppFile && !reactJsAppFile.content.includes("import Header from")) {
      setFiles(
        files.map((file) => {
          if (file.id === "react-js-1") {
            return {
              ...file,
              content: `// Ejemplo de componente React con JavaScript
...
ReactDOM.render(<App />, document.getElementById('root'));`,
    ...
    // Actualizar el archivo App.tsx para incluir importaciones
    const reactTsAppFile = files.find((file) => file.id === "react-ts-1")
    if (reactTsAppFile && !reactTsAppFile.content.includes("import Header from")) {
      setFiles(
        files.map((file) => {
          if (file.id === "react-ts-1") {
            return {
              ...file,
              content: `// Ejemplo de componente React con TypeScript
import Header from './Header';
...
function App(): JSX.Element {
  ...
  return (
    <div className="container">
      <Header title="React con TypeScript Demo" />
...
ReactDOM.render(<App />, document.getElementById('root'));`,
    ...
    // Añadir un archivo CSS con estilos para Tailwind
    const tailwindCssFile = {
      id: "css-2",
      name: "tailwind-example.css",
      content: `/* Estilos adicionales que complementan a Tailwind */
.header {
  @apply bg-gray-800 text-white p-4 mb-6;
}
...
/* Estos estilos se aplicarán cuando Tailwind esté activado */
`,
      language: "css",
      type: "css",
    }
    ...
    if (!files.some((file) => file.id === "css-2")) {
      setFiles((prev) => [...prev, tailwindCssFile])
    }

    // Añadir un archivo React con componentes Tailwind
    const tailwindReactFile = {
      id: "react-js-3",
      name: "TailwindDemo.jsx",
      content: `// Ejemplo de componente React con Tailwind CSS
  ...
  return (
    <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl m-4">
      <div className="md:flex">
        ...
        <div className="p-8">
          <div className="uppercase tracking-wide text-sm text-indigo-500 font-semibold">Tailwind CSS Demo</div>
          <h2 className="block mt-1 text-lg leading-tight font-medium text-black">Utilizando Tailwind en el Playground</h2>
          <p className="mt-2 text-gray-500">Activa el toggle de Tailwind CSS para ver cómo funciona este componente con clases de utilidad.</p>
            ...
            {isOpen && (
              <div className="mt-4 p-4 bg-gray-100 rounded">
                <p className="text-sm text-gray-700">
                  Tailwind CSS es un framework de CSS utilitario que te permite construir cualquier diseño directamente en tu HTML.
                </p>
...
export default TailwindDemo;`,
    ...
    // Verificar si el archivo ya existe
    if (!files.some((file) => file.id === "react-js-3")) {
      setFiles((prev) => [...prev, tailwindReactFile])
    }

    // Añadir un archivo React con componentes Tailwind en TypeScript
    const tailwindReactTsFile = {
      id: "react-ts-3",
      name: "TailwindDemo.tsx",
      content: `// Ejemplo de componente React con Tailwind CSS en TypeScript
interface TailwindDemoProps {
  title?: string;
}

function TailwindDemo({ title = "Tailwind CSS Demo" }: TailwindDemoProps): JSX.Element {
...
export default TailwindDemo;`,
    ...
    // Verificar si el archivo ya existe
    if (!files.some((file) => file.id === "react-ts-3")) {
      setFiles((prev) => [...prev, tailwindReactTsFile])
    }

    // Actualizar el archivo App.jsx para importar el componente TailwindDemo
    if (reactJsAppFile && !reactJsAppFile.content.includes("import TailwindDemo from")) {
      setFiles(
        files.map((file) => {
          if (file.id === "react-js-1") {
            return {
...
function App() {
  ...
  return (
    <div className="container">
      ...
      {/* Componente que usa Tailwind CSS */}
      <TailwindDemo />
...
ReactDOM.render(<App />, document.getElementById('root'));`,
    ...
    // Actualizar el archivo App.tsx para importar el componente TailwindDemo
    if (reactTsAppFile && !reactTsAppFile.content.includes("import TailwindDemo from")) {
      setFiles(
        files.map((file) => {
          if (file.id === "react-ts-1") {
            return {
...
function App(): JSX.Element {
  ...
  return (
    <div className="container">
      ...
      {/* Componente que usa Tailwind CSS */}
      <TailwindDemo title="TypeScript + Tailwind" />
